async function kmeansParallel(data,k,maxIterations,selecionado){
  ...
  let nunclusters;
  const sab = new SharedArrayBuffer(Float64Array.BYTES_PER_ELEMENT * data.length * k);
...
function initializeWorkers(k) {
  ...
  function updateCentroids(clusters, k){
    ...
    for(let i = 0; i < k; i++){
      console.log("valor dos clusters antes de entrar na calculateCentroid ",clusters);
  ...
  function createSharedArrayBuffer(dataArray,nunclusters) {
    sab = new SharedArrayBuffer(dataArray.length * nunclusters * Int32Array.BYTES_PER_ELEMENT * 5);
